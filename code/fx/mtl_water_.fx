...
out float4 outworldPos : TEXCOORD2)
{
	...
	// animate water
	float fCycle = vecTime.w * DoDefault(vecSkill41.y * 0.1, 0.1);
	float fAmp = DoDefault(vecSkill41.x * 0.001, 0.005);
	float fMod = 1 + DoDefault(vecSkill41.z, 25);
	
	outtex.x = intex1.x + fAmp * sin(intex1.y * fMod + fCycle);
	outtex.y = intex1.y - fAmp * sin(intex1.x * fMod + fCycle);
	outtex.zw = intex2.xy;
