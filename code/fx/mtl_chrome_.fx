...
sampler ChromeMap = sampler_state 
{
	Texture = <mtlSkin1>;
	Mipfilter = None;
	Minfilter = None;
...
out float2 outlookup: TEXCOORD3)
{
	inposition.w = 1.0f;
	...
	outlookup = mul(outnormal, matView) * facScale;
...
float2 lookup: TEXCOORD3) : COLOR0
{
	...
	// reflection power
	float facReflection = DoDefault(vecSkill41.y * 0.1, 1);
	
	// chrome
	float3 chrome = tex2D(ChromeMap, lookup) * facReflection;
	chrome *= vecSkill45.rgb;
