...
sampler MirrorTex = sampler_state
{
	Texture = <mtlSkin2>;
	MipFilter = Linear;
	MinFilter = Linear;
...
out float4 outworldPos : TEXCOORD3)
{
	...
	// vertex snapping
		...
		vertex.x = floor((vecSkill41.y + 40) * vertex.x) / (vecSkill41.y + 40); // default 160
		vertex.y = floor(vecSkill41.y * vertex.y) / vecSkill41.y; // default 120
	...
	outtex1.xy = intex1.xy;
	outtex1.zw = intex2.xy;
	
	// reflection coordinates
	outtex2 = outposition.xyw;
	...
	float light = 0;
	if(vecSkill41.w <= 0){ vecSkill41.w = 1; } // make sure we do have lights at all !
	for(light = 0; light < iLights; light += 1.0)
	{
		...
		// diffuse term
		outcolor.rgb += vecLightColor[light].rgb * saturate(-dot(ray, outnormal.xyz)) * strength * vecSkill41.w;
	}
	
	outcolor.rgb = saturate(outcolor.rgb);
	outcolor.rgb = pow(outcolor.rgb, vecSkill41.x); // adjustable brightness
...
float4 worldPos : TEXCOORD3) : COLOR0
{
	// reflection coordinates
	float2 Coord = 0.5 * (1.0 + tex2.xy / tex2.z);
	float4 mirrorColor = tex2D(MirrorTex, Coord);
	
	float4 textureColor = tex2D(ColorSampler, tex1.xy);
	...
	color.rgb *= textureColor.rgb + (mirrorColor.rgb * fAlbedo);
