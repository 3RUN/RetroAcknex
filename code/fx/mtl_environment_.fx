...
float4 worldPos : TEXCOORD2) : COLOR0
{
	...
	// environment mapping
	float3 cubeTexCoord = normalize(reflect(worldPos.xyz - vecViewPos, normal));
	float4 environment = texCUBE(EnvMapSampler, cubeTexCoord) * textureColor.w;	
	float facTex = DoDefault(vecSkill41.x * 0.01, 0.5);
	float4 resultingBase = lerp(environment, textureColor, facTex);
