...
#define ALPHA_GLOW // first skin's alpha is used for the glowing effect
//#define GLOW_WITHOUT_TEXTURE // ignore texture and add plain color to the glowing effect
...
float4 worldPos : TEXCOORD2) : COLOR0
{
	...
	// ugly but works :D
		...
		if(textureColor.a > 0)
		{
			#ifdef GLOW_WITHOUT_TEXTURE // no texture only glowing color ?
				color.rgb = normalize(vecSkill41.xyz) * textureColor.a;
				#else // only glowing texture ?
				color.rgb = vecSkill41.w * textureColor.a; // we can adjust glowing power by skill
				color.rgb *= textureColor.rgb * normalize(vecSkill41.xyz);
			#endif
		}
		else // no alpha?
		{
		...
		#else // no glow effect? then normal lighting !
		color.rgb *= textureColor.rgb;
	#endif
